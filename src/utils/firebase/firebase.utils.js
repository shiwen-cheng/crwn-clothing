...
export const addCollectionAndDocuments = async (
  ...
  const collectionRef = collection(db, collectionKey); // 有就找到，没有会自动创建
...
export const createUserDocumentFromAuth = async (
  ...
  const userDocRef = doc(db, "users", userAuth.uid); // 数据库，collection名，document名
...
export const onAuthStateChangedListener = (callback) =>
  onAuthStateChanged(auth, callback); // observer/listener， callback 会在每次 auth 改变时被调用
